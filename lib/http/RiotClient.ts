...
export class RiotClient implements RiotInterface {
  ...
  constructor(
    ...
    if (!RIOT_API_KEY) throw new Error('THROWING ERROR')
    this.RIOT_API_KEY = RIOT_API_KEY
  }
  /************************************************************************
   **************************** MATCH DATA ********************************
   ***********************************************************************/
  public async getTftByMatchId(matchId: string = ''): Promise<TFTMatch> {
  ...
  public async getAllTftMatchesByPuuid(encryptedPUUID: string = '', count = 20): Promise<Array<string>> {
    if (!encryptedPUUID) throw new Error('encryptedPUUID cannot be null')
    // wNAH--jmUok3Ln2Gbt0VHl_qfJhk8OJEFmC5o_fWyr3eD4D1e34x2WIfeGY8i_7kfmZ9fPrQtH-hCg
    const makeIds: Array<string> = await fetch(
      `https://${this.REGION_URL}/tft/match/v1/matches/by-puuid/${encryptedPUUID}/ids?count=${count}`,
      this.makeFetchOptions()
    )
    .then( (resp: any) => {
      return resp.json() 
    })
    .then( (jsonObj: Array<string>) => {
      return jsonObj
    })
    .catch( (err: any) => {
      // do logging
